function opts = prepareNUFFT3d(N, Nz, np,slthick,trajectory,viewOrder,varargin)
...
% all codes below are wrong. Multislice is not 3D!
    function prepare_spiral()
        ...
        sizeMask = size(mask);
        nufft_args = {sizeMask, [3 3 3], 2*sizeMask, sizeMask/2, 'table', 2^12, 'minmax:kb'};
    ...
    % xzc modified radial version. 2024-07-18
...
%                 ang = ang(angix);
        ...
        li = -N/2 : 0.5 : N/2-0.5;
        liz = -Nz/2:Nz/2-0.5; % append liz
        ky = li'*sind(ang);
        kx = li'*cosd(ang);
        kz = liz' ./ (opts.npar * opts.sltk);
        
        kxy = [kx(:)./opts.fov, ky(:)./opts.fov];
        % dcfxy = ir_mri_density_comp(kxy,'voronoi','fix_edge',0); % keep slice-by-slice
        % dcf_nz = repmat(dcfxy, Nz, 1);
        kspz = repelem(kz, nrow(kxy)); % [Nxy,Nz]
        omega = repmat(kxy, Nz, 1); % [Nxy*Nz,2]
        om = [omega, kspz(:)];
        [om, ~, ~] = unique(om, 'rows');
        ...
        maskSize = size(mask);
        % 3d
        nufft_args = {maskSize, [3 3 3], 2*maskSize, maskSize/2, 'table', 2^12, 'minmax:kb'};
        FOV = [opts.fov, opts.fov, opts.npar*opts.sltk];
        G = Gmri(om, mask, 'fov', FOV, 'basis', {'rect'}, 'nufft', nufft_args);
        wi = ir_mri_density_comp(om,'voronoi','fix_edge',0,'G',G.arg.Gnufft);
        % wi = ir_mri_density_comp(om);
        % wi = ir_mri_density_comp(om,'pipe', 'G', G.arg.Gnufft, 'fix_edge',0,'arg_pipe', {'fov', FOV, 'niter', 30});
        wib = reshape(wi,[size(kx),opts.npar]);
        wib(wib>0.00005) = 0;
        wib = wib(:);
